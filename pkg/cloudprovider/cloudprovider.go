...
func (cp *CloudProvider) Create(ctx context.Context, nodeClaim *karpv1.NodeClaim) (*karpv1.NodeClaim, error) {
	...
	server, err := cp.instanceProvider.Create(ctx, instance.CreateOpts{
		...
		NetworkIPRange:           nodeClass.Spec.NetworkIPRange,
		ServerNamePrefix:         nodeClass.Spec.ServerNamePrefix,
		NetworkIPBase:            nodeClass.Spec.NetworkIPBase,
		AdditionalNetworkIDs:     nodeClass.Spec.AdditionalNetworkIDs,
		AdditionalNetworkIPBases: nodeClass.Spec.AdditionalNetworkIPBases,
