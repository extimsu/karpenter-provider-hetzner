...
- Node network placement on `HCloudNodeClass`: `additionalNetworkIDs` attaches further private networks after `networkID`; `networkIPRange` pins the node to one subnet of the primary network; `serverNamePrefix` names servers and nodes `<prefix>-NN`, continuing the numbering of existing non-Karpenter servers; `networkIPBase` and `additionalNetworkIPBases` give each node the fixed IP base + NN on each network. Together they let Karpenter nodes join a cluster whose static workers follow a naming and addressing plan (firewall rules, allow-lists and egress routing keyed on IPs). All fields are optional and nothing changes when they are unset. CEL rules reject IP bases without `serverNamePrefix` and `networkIPRange`, and a server missing any `additionalNetworkIDs` network is drifted. (#95)
